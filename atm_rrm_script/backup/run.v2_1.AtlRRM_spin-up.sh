...
readonly PROJECT="e3sm" #"m3780"

# Simulation
readonly COMPSET="WCYCL20TR"
readonly RESOLUTION="atlanticx4v1pg2_r05_EC30to60E2r2"
...
readonly CASE_NAME="v2_1.${COMPSET}.${RESOLUTION}-SPINUP"
...
readonly drydep_in="/pscratch/sd/z/zhan391/WW3/ne32x4_mesh1/meshes/atmsrf_ne32x4_ATLpg2_20240411.nc"
readonly topo_in="/pscratch/sd/z/zhan391/WW3/ne32x4_mesh1/meshes/USGS-gtopo30_ne32x4_ATLpg2_12xdel2.nc"
...
readonly dtime=1800       #600
#readonly ATM_NCPL=96     #144 #48 #144 #86400/dtime 
readonly CLUBB_MACMIC=6   # 
readonly nu_top=2.5e5     #
readonly se_tstep=60 
readonly theta_hydrostatic_mode=.false.
readonly tstep_type=9
...
if [[ "${run}" != *"production"* ]] ;then
  ...
  readonly PELAYOUT="L"
  readonly WALLTIME="02:00:00"  #"48:00:00"
  readonly STOP_OPTION="ndays"  #"nyears"
  readonly STOP_N="5"           #"55"
  readonly REST_OPTION="ndays"  #"nyears"
  readonly REST_N="5"
  readonly RESUBMIT="2"
  readonly DO_SHORT_TERM_ARCHIVING=false
fi
...
cat << EOF >> user_nl_eam
 ...
 theta_hydrostatic_mode = ${theta_hydrostatic_mode}
 tstep_type             = ${tstep_type}

 nu_top = ${nu_top} 
 nu_div = 8.5e-8  !2.5*nu
 nu     = 3.4e-8  ! 
 nu_p   = 3.4e-8
 nu_q   = 3.4e-8
 nu_s   = 3.4e-8

 cld_macmic_num_steps  = ${CLUBB_MACMIC}
 dtime                 = ${dtime}
 se_tstep              = ${se_tstep}     ! dt_dyn    = tstep = 75s  
 dt_remap_factor       = 2               ! dt_remap  = tstep*dt_remap_factor         
 dt_tracer_factor      = 6               ! dt_tracer = tstep*dt_tracer_factor        
 hypervis_subcycle     = 1               ! dt_vis    = dt_dyn/hypervis_subcycle      
 hypervis_subcycle_q   = 1               ! dt_vis_q  = dt_tracer/hypervis_subcycle_q 
...
create_newcase() {
  ...
  #./xmlchange LND_NCPL=${ATM_NCPL}
  #./xmlchange OCN_NCPL=${ATM_NCPL}
