...
!Variables for machine learning bias correction 
  ...
  real(r8), allocatable :: Model_PSML(:,:,:)   !(Nudge_ncol,1,1)
  real(r8), allocatable :: Model_ZSML(:,:,:)   !(Nudge_ncol,1,1)
  real(r8), allocatable :: Model_IFRML(:,:,:)  !(Nudge_ncol,1,1)
  real(r8), allocatable :: Model_LFRML(:,:,:)  !(Nudge_ncol,1,1)
  real(r8), allocatable :: Model_OFRML(:,:,:)  !(Nudge_ncol,1,1)
  real(r8), allocatable :: Model_COZML(:,:,:)  !(Nudge_ncol,1,1)
  real(r8), allocatable :: Model_area(:)       !(Nudge_ncol)
...
!Variables for surface nudging 
  ...
  real(r8), parameter :: q_min            = 1.0e-12_r8 ! Minimum specific humidity
  real(r8), parameter :: q_max            = 1.0_r8 - q_min ! Maximum specific humidity
  real(r8), parameter :: q_denom_min      = 1.0e-8_r8  ! Minimum humidity in RH denominator
  real(r8), parameter :: numerical_tol    = 1.0e-12_r8 ! Near-zero numerical tolerance
...
contains
  ...
  subroutine nudging_readnl(nlfile)
   ...
   mltbc_file_template  = '%c_UNet_global_v4.pt'
  ...
  subroutine mltbc_validate_config()
    ...
    if (Nudge_Tv_Constrain_On .and. &
        (Nudge_Tprof == 0 .or. Nudge_Qprof == 0)) then
      call endrun('MLTBC: Tv constraint requires both T and Q nudging')
    end if
...
!DIAG
     ...
     write(iulog,*) 'NUDGING: mltbc_file_template =',mltbc_file_template 
     write(iulog,*) ' '
     write(iulog,*) ' '
...
!DIAG
   ...
   select case (Nudge_Method)
      ...
      case ('MLTBC')
           ...
           allocate(Model_area(Nudge_ncol),stat=istat)
           call alloc_err(istat,'Machine Learning NUDGING','Model_area',Nudge_ncol)
           ...
           call mpibcast(Model_area, Nudge_ncol, mpir8,   0, mpicom)
#endif
...
!        endif
     ...
     if ( Update_MLTBC ) then 
       ...
       if (mltbc_option == 4) then 
         ...
         do lchnk=begchunk,endchunk
           ...
           call zenith(calday,clat(1:ncol),clon(1:ncol),coszrs(1:ncol),ncol,dtime)
       ...
       call mltbc_calc_tend(pbuf2d,state,Nudge_ncol,nrows,mltbc_batch_dim, & !in
                            ...
                            Model_LFRML(:,:,1),Model_OFRML(:,:,1), & !in 
  ...
  !===========================================================================
  subroutine mltbc_calc_tend(pbuf2d,state,ngcols,ngrows,mltbc_ibdim, & !in 
                             uglb,vglb,tglb,qglb,psglb, & !in 
                             rlat,rlon,area,phis, & !in 
  ...
  if ( Nudge_Tprof .ne. 0  ) then
    select case (ndg_t_opt)
       ...
       case (2)
         ...
         do k = 1, pver
          do i = 1, ncol
            if ( (qcur(i,k) > q_denom_min) .and. (dqsdT_cur(i,k) > 0._r8) ) then
              nudge_t(i,k) = - (rhtp1(i,k) - rhcur(i,k)) / dtime & 
                             * qscur(i,k) * qscur(i,k) &
                             / (qcur(i,k)*dqsdT_cur(i,k))
            else
              nudge_t(i,k) = (tvtp1(i,k) - tvcur(i,k)) / dtime &
                             /(1.0_r8 + zvir * qcur(i,k))
            end if
  ...
  !===========================================================================
  ! Enforce humidity bounds while preserving the local ML virtual-temperature
  ! target through a compensating temperature adjustment.
  !===========================================================================
  subroutine mltbc_enforce_tv_constrain(ncol, dtime, zvirv, tcur, qcur, &
                                        nudge_t, nudge_q)
    ...
    do k = 1, pver
      do i = 1, ncol
        ...
        !------------------------------------------------------------
        if (isnan(tcur(i,k)) .or. isinf(tcur(i,k)) .or. &
            tcur(i,k) <= 100.0_r8) then
          call endrun('MLTBC constraint received invalid temperature')
        end if

        if (isnan(qcur(i,k)) .or. isinf(qcur(i,k)) .or. &
            qcur(i,k) < 0.0_r8 .or. qcur(i,k) >= 1.0_r8) then
          call endrun('MLTBC constraint received invalid humidity')
        end if

        if (isnan(zvirv(i,k)) .or. isinf(zvirv(i,k)) .or. &
            abs(zvirv(i,k)) <= numerical_tol) then
          call endrun('MLTBC constraint received invalid zvirv')
        end if

        !------------------------------------------------------------
        ! Raw ML target
        !------------------------------------------------------------
        if (isnan(nudge_t(i,k)) .or. isinf(nudge_t(i,k)) .or. &
            isnan(nudge_q(i,k)) .or. isinf(nudge_q(i,k))) then
          call endrun('MLTBC constraint received non-finite tendency')
        end if

        t_target = tcur(i,k) + nudge_t(i,k) * dtime
        q_target = qcur(i,k) + nudge_q(i,k) * dtime

        if (isnan(t_target) .or. isinf(t_target) .or. &
            t_target <= 100.0_r8) then
          call endrun('MLTBC constraint produced invalid T target')
        end if

        if (isnan(q_target) .or. isinf(q_target)) then
          call endrun('MLTBC constraint produced non-finite Q target')
        end if

        !------------------------------------------------------------
        ! Virtual-temperature target implied by the original ML
        ! prediction. This is the quantity we preserve if Q must
        ! be physically limited.
        !------------------------------------------------------------
        tv_target = t_target * &
                    (1.0_r8 + zvirv(i,k) * q_target)

        if (isnan(tv_target) .or. isinf(tv_target) .or. &
            tv_target <= 100.0_r8) then
          call endrun('MLTBC constraint produced invalid Tv target')
        end if

        !------------------------------------------------------------
        ! If Q is valid, retain the original ML prediction.
        !------------------------------------------------------------
        if (q_target >= q_min .and. q_target <= q_max) cycle

        !------------------------------------------------------------
        ! Q target is unphysical. Limit Q and, when T correction is
        ! active, compensate T to preserve the ML Tv target.
        !------------------------------------------------------------
        q_bal = min(q_max, max(q_min, q_target))

        denom = 1.0_r8 + zvirv(i,k) * q_bal

        if (isnan(denom) .or. isinf(denom) .or. &
            abs(denom) <= numerical_tol) then
          call endrun('MLTBC constraint has invalid Tv denominator')
        end if

        t_bal = tv_target / denom

        if (isnan(t_bal) .or. isinf(t_bal) .or. &
            t_bal <= 100.0_r8) then
          call endrun('MLTBC constraint produced invalid balanced T')
        end if

        nudge_t(i,k) = (t_bal - tcur(i,k)) / dtime
  ...
  subroutine nudging_timestep_tend(phys_state,phys_tend, dtime)
   ...
   use ppgrid       ,only: pver,pverp,pcols,begchunk,endchunk
   use cam_history  ,only: outfld
   use physconst    ,only: rga, cpair, gravit, zvir, cappa
   use hycoef       ,only: hycoef_init, hyam, hybm, hyai, hybi, ps0
   use infnan       ,only: isnan,isinf
   ...
   if(Nudge_ON) then
     ...
     ! Enforce humidity bounds against the live application-time state after
     ! all ML weighting and profile operations. Compensate temperature to
     ! preserve the local ML virtual-temperature target when both are active.
     if (mltbc_enabled .and. Nudge_Tv_Constrain_On) then
       zvirv(:ncol,:pver) = zvir
       call mltbc_enforce_tv_constrain(ncol, dtime, zvirv, &
                                       phys_state%t, phys_state%q(:,:,indw), &
...
!  call scatter_field_to_chunk(1,         1,1,Nudge_nlon,PSanal,Target_PS)
  ...
  real(r8), intent(in) :: uv_prelx
  real(r8), intent(in) :: t_prelx
  real(r8), intent(in) :: q_prelx
  ...
  !===========================================================================
   ...
   real(r8) :: ttend(ngcol,nlev)
   real(r8) :: qtend(ngcol,nlev)
   ...
   select case (mltbc_option)
     case (0)
       ...
       if (Nudge_Vprof .ne. 0 ) then
         call mltbc_aec_deeponet("V",nlev,mltbc_patch_nlon,mltbc_patch_nlat,ngcol,vpatch,vtend, & 
     ...
     case (1) 
       ...
       if (Nudge_Vprof .ne. 0 ) then
         call mltbc_state_deeponet("V",nlev,mltbc_patch_nlon,mltbc_patch_nlat,ngcol,vpatch,vtend, & 
     ...
     case (2)
       ...
       if (Nudge_Vprof .ne. 0 ) then
         call mltbc_single_model("V",nlev,mltbc_patch_nlon,mltbc_patch_nlat,ngcol, & 
                                 vpatch,vtend,kbot=1,ktop=nlev) 
       end if
     case default             
   ...
   call scatter_field_to_chunk(1,nlev,1,ngcol,ttend,nudge_t)
   call scatter_field_to_chunk(1,nlev,1,ngcol,qtend,nudge_q)

   return
  end subroutine !mltbc_advance_patch 

  subroutine mltbc_global_to_patch(nlev,ngcol,u,v,nlon,nlat,bilinear_interp,upatch,vpatch) !out 
  !===========================================================================
  ! SZ - 10/04/2024: This subroutine attempt to convert data in global region 
  !                  to array over a patch region 
  !===========================================================================
   use ppgrid,           only : pver,pverp,pcols,begchunk,endchunk
   use phys_grid,        only : scatter_field_to_chunk, get_ncols_p
   use shr_const_mod,    only : SHR_CONST_PI, SHR_CONST_REARTH
   ...
   real(r8),intent(in)          :: u(ngcol,nlev)
   real(r8),intent(in)          :: v(ngcol,nlev)

   real(r8),intent(inout)       :: upatch(nlon,nlat,nlev)
   real(r8),intent(inout)       :: vpatch(nlon,nlat,nlev)

   !local array 
   integer  :: k, lchnk

   real(r8) :: utem(pcols,pver,begchunk:endchunk)
   real(r8) :: vtem(pcols,pver,begchunk:endchunk) 

   real(r8) :: uerr(ngcol,nlev)
   real(r8) :: verr(ngcol,nlev)

   !regrid from global model grid to patch grid
   do k = 1, nlev
     upatch(:,:,k) = mltbc_global_to_latlon(ngcol,u,nlon,nlat,bilinear_interp)
     vpatch(:,:,k) = mltbc_global_to_latlon(ngcol,v,nlon,nlat,bilinear_interp)
   end do

   !Test the interpolation method 
   if (mltbc_bilerp_test) then
     do k = 1, nlev
       uerr(:,k) = mltbc_latlon_to_global(nlon,nlat,upatch(:,:,k),ngcol,bilinear_interp)
       verr(:,k) = mltbc_latlon_to_global(nlon,nlat,vpatch(:,:,k),ngcol,bilinear_interp)
     end do
  ...
  subroutine mltbc_advance_global(nlev,ngcol,ngrow,ibatch_dim,u,v,t,q,ps, & !in 
                                  cosz,icefrc,lndfrc,ocnfrc, & !in 
                                  phis,rlat,rlon,area, & !in 
                                  nudge_u,nudge_v,nudge_t,nudge_q,nudge_ps) !out
   !===========================================================================
   ...
   real(r8),intent(in)      :: rlat(ngcol)         !radians
   real(r8),intent(in)      :: rlon(ngcol)         !radians
   real(r8),intent(in)      :: area(ngcol)         !radians squared  
   real(r8),intent(in)      :: phis(ngcol,ngrow)   !m2/s2 
   real(r8),intent(in)      :: cosz(ngcol,ngrow)   !unitless [-1,1]
   real(r8),intent(in)      :: icefrc(ngcol,ngrow) !unitless [0,1]
   real(r8),intent(in)      :: lndfrc(ngcol,ngrow) !unitless [0,1]
   real(r8),intent(in)      :: ocnfrc(ngcol,ngrow) !unitless [0,1]
   ...
   select case (mltbc_option)
     ...
     case (3)
       ...
       if (Nudge_Vprof .ne. 0 ) then
         call mltbc_scalar_model("V",nlev,ngcol,ngrow,ngcol,ibatch_dim,u,v,t,q,vtend,lat,lon)
       end if
       if (Nudge_Tprof .ne. 0 ) then
         call mltbc_scalar_model("T",nlev,ngcol,ngrow,ngcol,ibatch_dim,u,v,t,q,ttend,lat,lon)
       end if
       if (Nudge_Qprof .ne. 0 ) then
     ...
     case (4)
       ...
       if (Nudge_Vprof .ne. 0 ) then
         call mltbc_scalar_model("V",nlev,ngcol,ngrow,ngcol,ibatch_dim,u,v,t,q,vtend, & 
                                 lat,lon,area,cosz,ocnfrc,icefrc,lndfrc,phis)
       end if
       if (Nudge_Tprof .ne. 0 ) then
         call mltbc_scalar_model("T",nlev,ngcol,ngrow,ngcol,ibatch_dim,u,v,t,q,ttend, & 
                                 lat,lon,area,cosz,ocnfrc,icefrc,lndfrc,phis)
       end if
       if (Nudge_Qprof .ne. 0 ) then
         call mltbc_scalar_model("Q",nlev,ngcol,ngrow,ngcol,ibatch_dim,u,v,t,q,qtend, & 
                                 lat,lon,area,cosz,ocnfrc,icefrc,lndfrc,phis)
       end if
     case default
  ...
  subroutine mltbc_scalar_model(var,nz,nx,ny,ngtot,ibdim,u,v,t,q,tend, &
                                lat,lon,area,cosz,ocnfrc,icefrc,lndfrc,phis)
   !===========================================================================
   ! SZ - 06/05/2023: This subroutine attempt to call the forecast for
   !                  the Machine Learning (ML) model,
   ...
   real(r8),intent(in),optional  :: lat(nx)
   real(r8),intent(in),optional  :: lon(nx)
   real(r8),intent(in),optional  :: area(nx)
   real(r8),intent(in),optional  :: cosz(nx,ny)
   real(r8),intent(in),optional  :: phis(nx,ny)
   real(r8),intent(in),optional  :: ocnfrc(nx,ny)
   real(r8),intent(in),optional  :: lndfrc(nx,ny)
   real(r8),intent(in),optional  :: icefrc(nx,ny)
   ...
   call t_startf ('mltbc_scalar_model_forward')
   if (trim(var) == "U" ) then
     call torch_umod%forward(input_tensors,out_tensor)
   else if (trim(var) == "V" ) then
     call torch_vmod%forward(input_tensors,out_tensor)
  ...
  subroutine mltbc_aec_deeponet(var,nz,nx,ny,ngtot,xstate,tend, &
  ...
  !===========================================================================
   ...
   character(len=*),intent(in)  :: var ! variable to be predicted 
   integer, intent(in)          :: nx,ny,nz,ngtot
   ...
   call mltbc_deeponet_encoder(var,mltbc_patch_model, & 
  ...
  subroutine mltbc_state_deeponet(var,nz,nx,ny,ngtot,vari,tend,ktop,kbot,xmin,xmax)
  !=================================================================================
  ! SZ - 06/05/2023: This subroutine attempt to call the forecast for 
  ...
  !=================================================================================
   ...
   if (mltbc_patch_model) then 
     ...
     do k = k1, k2
       tend(1:ngtot,k) = mltbc_latlon_to_global(nx,ny,varo(:,:,k),ngtot,mltbc_patch_bilerp)
     end do
     deallocate(input2d,input4d)

   else !global model 

     call endrun('Machine Learning Nudging Error: correction on state only trained for patch...')
  ...
  subroutine mltbc_deeponet_encoder(var,l_ml_patch,nx,ny,nz,nx1,ny1,nz1, &
                                    vari,varo,k1,k2,vmin,vmax)
  !===========================================================================
  ...
  subroutine mltbc_deeponet_decoder(var,l_ml_patch,nx1,ny1,nz1,nx,ny,nz,& 
  ...
  function mltbc_global_to_latlon(nxy, xglb, nx, ny, bilinear_interp) result (xpatch) 
   !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  function mltbc_latlon_to_global(nx, ny, xpatch, nxy, bilinear_interp) result (xglb)
   !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
